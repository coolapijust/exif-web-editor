...
import { writeImage, clearAllExif, clearTag } from '@/services'
...
export function useExif() {
  ...
  async function loadExifData(files: File[]): Promise<void> {
    ...
    try {
      ...
      if (results.length > 0 && !fileStore.selectedId) {
        fileStore.selectFile(results[0].id)
      }
    } catch (e) {
  ...
  async function updateTag(tagName: string, value: unknown): Promise<void> {
    ...
    try {
      ...
      if (!file) return
      await writeImage(file, { [tagName]: value } as Partial<ExifData>)
  ...
  async function removeTag(tagName: string): Promise<void> {
    ...
    try {
      const file = currentFile.value
      if (!file) return
      await clearTag(file, tagName)
      exifStore.removeTag(fileStore.selectedId, tagName)
      uiStore.showNotification({
        id: Date.now().toString(),
        type: 'success',
  ...
  async function clearAllTags(): Promise<void> {
    ...
    try {
      const file = currentFile.value
      if (!file) return
      await clearAllExif(file)
      const currentData = exifStore.data[fileStore.selectedId]
      if (currentData) {
        Object.keys(currentData).forEach(key => {
          if (key !== 'fileName' && key !== 'fileSize' && key !== 'mimeType') {
            exifStore.removeTag(fileStore.selectedId!, key)
          }
        })
      }
      uiStore.showNotification({
