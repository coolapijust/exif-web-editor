...
async function downloadModifiedFile(): Promise<void> {
    if (!modifiedFileData.value || !selectedFile.value || !fileStore.selectedId) return
    ...
    await fileDb.saveFile({
      ...
      content: modifiedFileData.value,
      preview: selectedFile.value.preview || null,
      exifData: exifData.value
    })

    modifiedFileData.value = null
  }
</script>
