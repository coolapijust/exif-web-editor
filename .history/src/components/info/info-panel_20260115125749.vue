...
const DISPLAY_GROUPS = new Set(['GPS', 'Photo'])
    ...
    <template v-if="selectedFile">
      ...
      <div class="p-6 space-y-6">
        ...
        <template v-for="(tags, groupName) in groupedTags" :key="groupName">
          <TagGroup
            v-if="DISPLAY_GROUPS.has(groupName)"
        ...
        <div v-if="Object.values(groupedTags).every(tags => !DISPLAY_GROUPS.has(Object.keys(groupedTags).find(key => groupedTags[key] === tags) || '')) && Object.keys(groupedTags).length > 0" class="text-center py-8">
