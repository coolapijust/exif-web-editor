...
const emit = defineEmits<{
  close: []
  save: [tagName: string, value: string | string[]]
}>()
...
function handleSave(): void {
  ...
  emit('save', props.tagName, finalValue)
}
</script>
...
<template>
  <Teleport to="body">
    ...
    >
      <div
        v-if="open"
        class="fixed inset-0 bg-gray-900/30 flex items-center justify-center z-50"
            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center gap-2">
              ...
              <div class="relative">
                <button
                  ...
                  class="w-5 h-5 rounded-full bg-gray-200 dark:bg-gray-600 text-gray-600 dark:text-gray-300 text-xs font-medium flex items-center justify-center hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors"
                >
                  ?
                </button>
                <Transition
                ...
                >
                  <div
                    v-if="showTooltip"
                    class="absolute left-full top-0 ml-2 w-64 p-3 bg-gray-900 dark:bg-gray-700 text-white text-sm rounded-lg shadow-lg z-50"
                  >
                    {{ t('editor.editTip') }}
                    <div class="absolute left-0 top-1/2 -translate-x-1 -translate-y-1/2 w-2 h-2 bg-gray-900 dark:bg-gray-700 rotate-45"></div>
                  </div>
                </Transition>
              </div>
            </div>
